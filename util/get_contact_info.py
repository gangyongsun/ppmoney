...
# 总配置信息（需要用户自己收集）
csv_file_name = '债权明细配置信息.csv'
csv_file = csv.reader(open('../csv/' + csv_file_name, 'r', encoding='utf-8-sig'))
# 每页条数（最大100，再调高无用）
pageSize = 100
...
for line_data in csv_file:
    ...
    for i in range(int(totalPageNum)):
        ...
        # 组合【查看债权明细】弹出页面的URL
        url = credit_url_1 + str(id_value) + credit_url_2 + str(page_index) + credit_url_3 + str(pageSize) + credit_url_4 + str(project_type_value) + CONFIG.credit_timestamp + str(timestamp)
        ...
        for item in list_info:
            ...
            # 出借人合同完整URL
            borrower_contact_url = cat_url_1 + str(investId) + cat_url_2 + asset_id + cat_url_3 + shar_id + cat_url_4 + str(_type) + cat_url_5 + str(project_type) + cat_url_6 + str(assetType)
        ...
        credit_df.to_csv('./' + name + str(page_index) + '.csv', encoding='utf_8_sig')
